...
export default function JourneyCard({ journey, showJoinButton = true, compact = false, loginRedirect = false }: JourneyCardProps) {
  ...
  return (
    <Link
      href={href}
      className="group block rounded-2xl border border-ink/[0.09] bg-surface p-5 shadow-[0_2px_16px_rgba(28,22,16,0.10)] hover:shadow-[0_4px_24px_rgba(28,22,16,0.15)] hover:border-brand/[0.22] transition-all duration-200"
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-ink/55 mt-0.5 flex items-center gap-1">
            <span>For</span>
            <span className="font-medium text-ink/70">{journey.dedication_name}</span>
          </p>
        </div>
        <ArrowRight className="h-4 w-4 text-ink/30 group-hover:text-brand shrink-0 mt-1 transition-colors" />
      </div>
